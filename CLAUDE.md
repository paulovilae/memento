...
- If **genesis** dies, all Memento data survives on anchor's standby (RPO ≈ 0). **Failover is AUTOMATIC (since 2026-06-06, corrected here 2026-07-04)**: the `db-failover-watchdog` on anchor (`scripts/db_health_watchdog.sh`, declared in `anchor.ecosystem.config.cjs`) probes genesis every 20 s over two routes and, after ~2 min of confirmed outage corroborated by the laptop quorum witness (`scripts/witness_genesis.py:9099`), runs `scripts/db_failover_promote.sh` — promotes the standby and starts `memento-node` on anchor. `docs/DB_FAILOVER_RUNBOOK.md` documents the assisted-manual fallback and the kill-switch (`/tmp/disable-db-failover`). Still manual: re-pointing apps to the new primary (runbook §"repunte").
