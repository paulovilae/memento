...
pub async fn start_folder_watcher(folders: Vec<WatchedFolder>) -> anyhow::Result<()> {
    info!("👀 Starting Local Folder Watcher on {} configured directories", folders.len());
    ...
    tokio::spawn(async move {
        ...
        for folder in folders {
            info!("🔍 Scanning {} for documents... (Sanitize: {})", folder.path, folder.sanitize_pii);
                        ...
                        match ext_str.as_str() {
                            "pdf" => {
                                match pdf_extract::extract_text(p) {
                                    Ok(text) => info!("📄 Added PDF to Queue: {} ({} bytes)", p.display(), text.len()),
                                    Err(e) => warn!("⚠️ Failed to extract PDF {}: {}", p.display(), e),
                                }
                            }
                            "docx" => {
                                match extract_docx(p) {
                                    Ok(text) => info!("📝 Added DOCX to Queue: {} ({} bytes)", p.display(), text.len()),
                                    Err(e) => warn!("⚠️ Failed to extract DOCX {}: {}", p.display(), e),
                                }
                            }
                            "xlsx" => {
                                match extract_xlsx(p) {
                                    Ok(text) => info!("📊 Added XLSX to Queue: {} ({} bytes)", p.display(), text.len()),
                                    Err(e) => warn!("⚠️ Failed to extract XLSX {}: {}", p.display(), e),
                                }
                            }
                            _ => {
        ...
        info!("🛡️ Scan complete. {} documents pending SLM PII sanitization.", document_count);
            ...
            for row in range.rows() {
                for cell in row {
                    match cell {
                        ...
                        Data::Bool(b) => { text.push_str(&b.to_string()); text.push(' '); },
                        _ => {}
                    }
                }
                text.push('\n');
