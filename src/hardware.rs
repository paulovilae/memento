...
pub fn discover_hardware() -> ComputeProfile {
    ...
    let (has_gpu, gpu_name, strategy) = if has_nvidia {
        (
            true,
            Some("NVIDIA RTX (CUDA)".to_string()),
            "VRAM Fast Path (FastEmbed + SLM)".to_string(),
        )
    } else if has_metal {
        (
            true,
            Some("Apple Silicon (Metal)".to_string()),
            "Metal Unified Memory".to_string(),
        )
    } else {
    ...
    if profile.has_gpu {
        info!(
            "⚡ GPU Detected: {}. Enabling Liquid Compute Fast Path.",
            profile.gpu_name.as_ref().unwrap()
        );
    } else {
